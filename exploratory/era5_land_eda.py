...
def ejecutar_bloque3_eda():
    path_horario = Path("era5_land_merida_2020_2026_utc4.parquet")
    path_diario = Path("era5_land_merida_diario_2020_2026.parquet")
    ...
    anual_tp = df_h.groupby("month")["tp_mm"].sum() / df_h["time_utc4"].dt.year.nunique()  # Acumulado promedio mensual
    ...
    ax1 = axes[0, 0]
    ax1_wind = ax1.twinx()
    l1 = ax1.plot(diurno_t2m.index, diurno_t2m.values, color='#e74c3c', lw=2.5, label='Temperatura 2m (°C)')
    l2 = ax1_wind.plot(diurno_wind.index, diurno_wind.values, color='#2980b9', lw=2.5, linestyle='--', label='Velocidad Viento (m/s)')
    ax1.set_title('Ciclo Diurno Promedio: Temperatura y Viento (UTC-4 VET)', fontweight='bold')
    ...
    ax3 = axes[1, 0]
    ax3_t = ax3.twinx()
    b1 = ax3.bar(anual_tp.index, anual_tp.values, color='#2ecc71', alpha=0.7, label='Precipitación Mensual (mm)')
    l3 = ax3_t.plot(anual_t2m.index, anual_t2m.values, color='#c0392b', lw=2.5, marker='o', label='Temperatura Media (°C)')
    ax3.set_title('Ciclo Anual / Estacionalidad: Lluvias y Temperatura', fontweight='bold')
    ...
    ax4 = axes[1, 1]
    ax4_rad = ax4.twinx()
    l4 = ax4.plot(anual_swvl.index, anual_swvl.values, color='#8e44ad', lw=2.5, marker='s', label='Humedad Suelo (m³/m³)')
    l5 = ax4_rad.plot(anual_ssrd.index, anual_ssrd.values, color='#f39c12', lw=2.5, linestyle=':', marker='^', label='Radiación Solar (W/m²)')
    ...
    plt.tight_layout()
    fig_ciclos_path = "ciclos_temporales_merida.png"
    ...
    pivot_tp = df_valid.pivot(index="date", columns=["latitude", "longitude"], values="tp_daily_sum")
    pivot_t2m = df_valid.pivot(index="date", columns=["latitude", "longitude"], values="t2m_mean")
    ...
    for mode in range(3):
        ...
        im = axes[mode].imshow(grid_eof, cmap='RdBu_r', extent=[min(unique_lons), max(unique_lons), min(unique_lats), max(unique_lats)], aspect='auto')
        axes[mode].set_title(f'Modo Spatial EOF{mode+1} ({var_exp_tp[mode]:.1f}% Varianza)', fontweight='bold')
        axes[mode].set_xlabel('Longitud (°W)')
        axes[mode].set_ylabel('Latitud (°N)')
        fig.colorbar(im, ax=axes[mode], label='Amplitud Normalizada')

    plt.tight_layout()
    fig_eof_spatial_path = "eof_patrones_espaciales.png"
    plt.savefig(fig_eof_spatial_path, dpi=300)
    plt.close()
    print(f"  -> Gráfico de patrones espaciales EOF guardado en: '{fig_eof_spatial_path}'")
    ...
    for mode in range(3):
        axes[mode].plot(dates_idx, pcs_tp[:, mode], color=colors[mode], lw=1.2, label=f'PC{mode+1} ({var_exp_tp[mode]:.1f}%)')
    ...
    axes[0].set_title('Series Temporales de Componentes Principales (PCs - Precipitación Mérida)', fontweight='bold')
    axes[2].set_xlabel('Fecha (2020 - 2026)')
    plt.tight_layout()
    fig_eof_pcs_path = "eof_pcs_temporales.png"
    ...
    cols_var_diarias = [c for c in df_valid.columns if c not in ["date", "latitude", "longitude"]]
    ...
    vars_clave = [
        "tp_daily_sum", "t2m_mean", "t2m_min", "t2m_max", "d2m_mean",
        "swvl1_mean", "swvl2_mean", "swvl3_mean", "sp_mean",
        "u10_mean", "v10_mean", "ssrd_mean", "strd_mean",
        "sshf_mean", "slhf_mean", "e_mean", "evavt_mean"
    ]
    ...
    ax.set_title('Matriz de Correlación de Pearson (Variables Climáticas Mérida)', fontweight='bold', pad=20)
    ...
    plt.tight_layout()
    fig_corr_path = "matriz_correlacion_multivariable.png"
