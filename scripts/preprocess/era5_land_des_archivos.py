...
def descomprimir_y_fusionar_era5(directorio_era5="data_era5land", borrar_zip=True):
    ...
    for i, zip_file in enumerate(archivos_zip, 1):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                nc_internos = [os.path.join(tmpdir, f) for f in os.listdir(tmpdir) if f.endswith(".nc")]
                
                if not nc_internos:
                    print(f"[{i}/{total}] Advertencia: No hay archivos .nc dentro de {zip_file.name}")
                    continue
